...
internal sealed class WebSocketMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, IWebSocketRpcSession session)
    {
        ...
        this.logger.LogDebug("WebSocket Client Connected: Client ID: '{ClientId}', Client IP: '{ClientIp}'", clientId, clientIp);
        ...
        try
        {
            ...
            this.logger.LogInformation("Starting RPC session: Client ID: '{ClientId}', Client IP: '{ClientIp}'", clientId, clientIp);
            await session.RunAsync(socket, context.RequestAborted).ConfigureAwait(false);
            this.logger.LogInformation("WebSocket Client Disconnected: Client ID: '{ClientId}', Client IP: '{ClientIp}'", clientId, clientIp);
        }
        catch (OperationCanceledException) when (context.RequestAborted.IsCancellationRequested)
        {
            //// Client disconnected abruptly (closed the window, network dropped, etc).
            this.logger.LogInformation("WebSocket Client Disconnected Abruptly. ConnectionId: {ConnectionId}, IP: {ClientIp}", clientId, clientIp);
        }
        finally
        {
