...
internal sealed class AccessTokenHandlerTests
{
    ...
    {
        // Arrange
        var session = new UserSession(Guid.NewGuid(), token, DateTimeOffset.UtcNow.AddSeconds(5));
    ...
    {
        // Arrange
        var session = new UserSession(Guid.NewGuid(), "new-token", DateTimeOffset.UtcNow.AddSeconds(5));
        userSessionContext.UserSession.Returns(session);
        serviceProvider.GetService(typeof(IUserSessionContext)).Returns(userSessionContext);
    ...
    {
        // Arrange
        var session = new UserSession(Guid.NewGuid(), "valid-token", DateTimeOffset.UtcNow.AddSeconds(5));
        userSessionContext.UserSession.Returns(session);
        serviceProvider.GetService(typeof(IUserSessionContext)).Returns(userSessionContext);
