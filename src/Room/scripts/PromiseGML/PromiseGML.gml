...
function __Promise(_handler/*:function*/) constructor {
	...
	static _logger = new Logger(nameof(__Promise));
	...
	/// @hint Promise:next(onFulfilled:function, ?onRejected:function)->Promise
	static next = function(_onFulfilled, _onRejected = undefined)/*->Promise*/ {
	...
	/// @hint Promise:fail(onRejected:function)->Promise
	static fail = function(_onRejected)/*->Promise*/ {
		return next(undefined, _onRejected);
	}
	
	/// @hint Promise:lastly(callback:function)->Promise
	static lastly = function(_callback) {
		with ({
			__callback: _callback,
			__value: undefined,
		}) {
			return other.next(function(_value) {
				__value = _value;
				return Promise_resolve(__callback()).next(function() {
					return __value;
				})
			}, function(_reason) {
				__value = _reason;
	...
	static __finale = function() {
		...
		if (self.__state == 2 && _len == 0) {
			ds_list_add(__Promise_soon, function() {
				if (!self.__handled) {
					_logger.log(log_type.error, $"Possible Unhandled Promise Rejection: {__value}");
				}
			});
		}
...
function __Promise_all_res(_args/*:array*/, _ind/*:int*/, _val/*:any*/, _resolve/*:function*/, _reject/*:function*/, _remaining/*:int[]*/) {
	try {
		if (is_struct(_val) && is_method(_val[$"next"])) {
			with ({
				__ind: _ind,
				__args: _args,
				__resolve: _resolve,
...
function Promise_race(_arr/*:array*/) {
	with ({__arr: _arr}) return new __Promise(function(_resolve, _reject) {
		...
		var _len = array_length(__arr);
		for (var _ind = 0; _ind < _len; _ind++) {
			Promise_resolve(__arr[_ind]).next(_resolve, _reject);
		}
	});
}
