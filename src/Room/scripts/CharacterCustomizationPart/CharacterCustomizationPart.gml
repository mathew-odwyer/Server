...
function CharacterCustomizationPart(type, max_variations, max_colors) constructor
{
	...
	/// @description Retrieves the data ready to be transfered over a socket.
	/// @returns {Struct} The data ready to be transfered over a socket.
    get_data = function()
	{
		return {
			variation_index: int64(_variation_index),
			color_index: int64(_color_index),
		};
	}
	
	/// @description Gets the sprite index for this customization part based on the current variation and color.
	/// @returns {Asset.GMSprite} Returns the sprite index for this customization part based on the current variation and color.
	get_sprite = function()
	{
