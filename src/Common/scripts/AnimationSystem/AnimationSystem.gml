...
{
    /// @type {Id.DsGrid<String, Struct.Animation>}
    /// @description The tag to animation map.
    _tag_to_animation_map = ds_map_create();
    ...
    /// @type {Bool}
    /// @description Indicates whether this struct has released it's resources.
    _is_free = false;
    ...
    {
        if (ds_map_exists(_tag_to_animation_map, tag))
        {
            throw $"The specified {tag} has already been registered to an {nameof(Animation)} for this {nameof(AnimationSystem)}.";
        } 

        _tag_to_animation_map[? tag] = animation;
    }

    /// @description Frees the resources allocated by this struct.
    free = function()
    {
        if (_is_free)
        {
        ...
        if (!is_undefined(_tag_to_animation_map))
        {
            ds_map_destroy(_tag_to_animation_map);
            _tag_to_animation_map = undefined;
        }
