...
public sealed class GetPlayerRequestHandler : IRequestHandler<GetPlayerRequest, GetPlayerResponse>
{
    ...
    /// <inheritdoc/>
    public Task<GetPlayerResponse> Handle(GetPlayerRequest request, CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(new GetPlayerResponse(this.mapper.Map<PlayerDto>(player)));
    }
}
